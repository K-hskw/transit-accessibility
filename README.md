...
ただし**網走市のメッシュ人口データは入手できていない**ため、実際の空白人口の算出は未実施です。
...
├── test_all.py / _v2 / _v3   # 自動テスト（474 / 137 / 286件）
├── test_blank_area.py        # 自動テスト（24件）
├── test_day_type.py          # 自動テスト（13件）
