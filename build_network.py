...
# ===== 徒歩乗り換えエッジを作成 =====
# 300m以内のバス停間に徒歩エッジを追加（時速4km = 約67m/分）
...
for i in range(len(stop_list)):
    for j in range(i+1, len(stop_list)):
        ...
        if dist <= MAX_WALK_DIST and stop_list[i][0] != stop_list[j][0]:
            walk_time = int((dist / WALK_SPEED) * 60)  # 秒
            if walk_time < 1:
                walk_time = 1
            walk_edges.append({
...
# ===== 保存 =====
edges_df.to_csv("bus_edges.csv", index=False)
walk_df.to_csv("walk_edges.csv", index=False)
